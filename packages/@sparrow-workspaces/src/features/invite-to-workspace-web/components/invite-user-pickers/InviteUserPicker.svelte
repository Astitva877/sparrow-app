<script lang="ts">
   ...
   */
  export let onAdd: (email: string) => void;
  ...
  >
    <div class="email-container rounded d-flex flex-wrap">
      ...
      <input
        ...
        disabled={currentInstance.selectedEmail ?? false}
      />
    </div>
    {#if isOpen}
      <div class="select-data p-1 rounded" transition:slide={{ duration: 100 }}>
        {#each list as user, index}
          <div
            class="d-flex w-100 px-2 py-2 highlight rounded select-active"
            on:click={() => {
